...
public struct PhotosCondition: OperationCondition {
    ...
    public func evaluateForOperation(operation: Operation, completion: OperationConditionResult -> Void) {
        switch PHPhotoLibrary.authorizationStatus() {
            case .Authorized: completion(.Satisfied)
            default:
...
private class PhotosPermissionOperation: Operation {
    ...
    override func execute() {
        switch PHPhotoLibrary.authorizationStatus() {
            case .NotDetermined: dispatch_async(dispatch_get_main_queue()) { PHPhotoLibrary.requestAuthorization { status in self.finish() } }
            default: finish()
        }
    }
